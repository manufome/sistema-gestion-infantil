...
from students.models import Student, AcademicProgress
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if role in ['teacher', 'guardian']:
                user.save()
                group_name = 'Teacher' if role == 'teacher' else 'Guardian'
                group = Group.objects.get(name=group_name)
                user.groups.add(group)
                auth_login(request, user)
                messages.success(request, f'Bienvenido {user.username} a la plataforma')
...
class UserLoginView(LoginView):
    ...
    def get_success_url(self):
        messages.success(self.request, f'Bienvenido {self.request.user.username} a la plataforma')
...
def admin_dashboard(request):
    ...
    total_guardians = User.objects.filter(groups__name='Guardian').count()

    # Calcular nuevos registros en el último mes
    un_mes_atras = timezone.now() - timedelta(days=30)
    nuevos_students_mes = Student.objects.filter(fecha_registro__gte=un_mes_atras).count()
    nuevos_institutions = Institution.objects.filter(fecha_registro__gte=un_mes_atras).count()
    nuevos_teachers = User.objects.filter(groups__name='Teacher', date_joined__gte=un_mes_atras).count()
    nuevos_guardians_mes = User.objects.filter(groups__name='Guardian', date_joined__gte=un_mes_atras).count()

    # Obtener actividades recientes
    actividades_recientes = Actividad.objects.all().order_by('-fecha')[:5]

    context = {
        'total_niños': total_students,
        'total_jardines': total_institutions,
        'total_madres_comunitarias': total_teachers,
        'total_acudientes': total_guardians,
        'nuevos_niños_mes': nuevos_students_mes,
        'nuevos_jardines': nuevos_institutions,
        'nuevas_madres_comunitarias': nuevos_teachers,
        'nuevos_acudientes_mes': nuevos_guardians_mes,
...
@login_required
@user_passes_test(lambda user: user.is_guardian())
...
def panel_acudiente(request):
    user = request.user
    students = Student.objects.filter(guardian=user)
    if request.htmx:
        if request.GET.get('niño'):
            student = Student.objects.get(pk=request.GET.get('niño'))
            avances = AcademicProgress.objects.filter(student=student).order_by('-fecha_entrega')
            return render(request, 'acudiente/partials/avance_academico.html', {'avances': avances})
        if request.GET.get('publicacion'):
            posts = Post.objects.all()
            return render(request, 'partials/lista_publicaciones.html', {'publicaciones': posts})
    return render(request, 'acudiente/panel_acudiente.html', {'niños': students})
...
def publicaciones(request):
    posts = Post.objects.all()
    return render(request, 'madre/publicaciones.html', {'publicaciones': posts})
